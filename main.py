...
def atualizar():
    try: 
        ...
        botao_confirmar = Button(frameMeio, command= update, text="Confirmar".upper(), width=13, height=1,
...
def escolher_imagem():
    ...
    imagem = fd.askopenfilename() #pegando caminho da imagem

    imagem_string = imagem #Guardando o caminho da imagem no imagem_string

    imagem = Image.open(imagem) #abrindo a imagem pelo seu caminho
    imagem = imagem.resize((170, 170))  #Mudando o tamanho da imagem
    imagem = ImageTk.PhotoImage(imagem) #Convertendo a imagem para o formato que possamos usar no tkinter
...
def mostrar():
    ...
    for co1 in tabela_head:
        tree.heading(co1, text=co1.title(), anchor=CENTER)
